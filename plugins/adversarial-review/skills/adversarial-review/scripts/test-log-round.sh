...
test_state_has_round_1_data() {
    ...
    ADVERSARIAL_REVIEW_LOG="$LOG_FILE" \
        bash "$SCRIPT_DIR/log-round.sh" 1 "spec.md" "NO-GO" 7 3 1 "codex/gpt-5.5"
    local round
    round=$(python3 -c "import json; print(json.load(open('.adversarial-review/state.json'))['round'])")
    assert_eq "round=1" "1" "$round"
    local findings
    findings=$(python3 -c "import json; print(json.load(open('.adversarial-review/state.json'))['findings_total'])")
...
test_state_accumulates_rounds() {
    ...
    local history_len
    history_len=$(python3 -c "import json; print(len(json.load(open('.adversarial-review/state.json'))['history']))")
    assert_eq "history has 3 rounds" "3" "$history_len"
    local total_findings
    total_findings=$(python3 -c "import json; print(json.load(open('.adversarial-review/state.json'))['findings_total'])")
