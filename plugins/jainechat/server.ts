#!/usr/bin/env bun
/**
 * JAINE Chat for Claude Code.
 *
 * Localhost web UI for channel messaging. No external service,
...
const mcp = new Server(
  { name: 'jainechat', version: '0.1.0' },
  {
    capabilities: { tools: {}, experimental: { 'claude/channel': {} } },
    instructions: `The sender reads the jainechat UI, not this session. Anything you want them to see must go through the reply tool — your transcript output never reaches the UI.\n\nMessages from the jainechat web UI arrive as <channel source="jainechat" chat_id="web" message_id="...">. If the tag has a file_path attribute, Read that file — it is an upload from the UI. Reply with the reply tool. UI is at http://localhost:${PORT}.`,
  },
)
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'reply',
      description: 'Send a message to the jainechat UI. Pass reply_to for quote-reply, files for attachments.',
      inputSchema: {
        type: 'object',
        properties: {
          text: { type: 'string' },
...
form { position: fixed; bottom: 0; left: 0; right: 0; padding: 1em; background: #111; border-top: 1px solid #333; }
#text { width: 100%; box-sizing: border-box; font: inherit; margin-bottom: 0.5em; background: #1a1a1a; color: #e0e0e0; border: 1px solid #333; padding: 0.5em; }
#file { display: none; }
#row { display: flex; gap: 1ch; }
#row button { background: #222; color: #e0e0e0; border: 1px solid #444; padding: 0.3em 0.8em; cursor: pointer; }
#row button:hover { background: #333; }
#row button[type=submit] { margin-left: auto; background: #1a3a1a; border-color: #2a5a2a; }
...
function add(m) {
  const who = m.from === 'user' ? 'you' : 'jaine'
