...
The bulldozer plugin ships its OWN MCP server (`mcp/codex_server.py`) — v2 as of 2026-06-19. Three tools (dispatched by `params.name` in `main()`): **`codex_run`** (turn lifecycle — review/implement), **`codex_info`** (connection-level reads — `query`: models/auth/config/limits/usage/servers/features/profiles; no cold-start), **`codex_review`** (native git-aware `review/start` — `target`: uncommitted/branch/commit/custom → free-text findings). **Do NOT use the stock `codex mcp-server`** — see below.
