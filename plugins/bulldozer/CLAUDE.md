...
The bulldozer plugin ships its OWN MCP server (`mcp/codex_server.py`) — v2 as of 2026-06-19. Three tools (dispatched by `params.name` in `main()`): **`codex_run`** (turn lifecycle — review/implement), **`codex_info`** (connection-level reads — `query`: models/auth/config/limits/usage/servers/features/profiles; no cold-start), **`codex_review`** (native git-aware `review/start` — `target`: uncommitted/branch/commit/custom → free-text findings). **Do NOT use the stock `codex mcp-server`** — see below. The `initialize` reply also carries an `instructions` routing manifest (`SERVER_INSTRUCTIONS` → `_initialize_result`, #256) — CC injects `InitializeResult.instructions` into the model's context on connect (CC #30135), giving a server-level map to pick between the three tools (`codex_run`/`codex_info` are MCP-only — no skill covers them).
...
- **`error` notification handling (2026-06-21):** `_classify_error_notification` — `willRetry:true` = transient stream reconnect (codex retries; counted as `retries`, NOT `_drift`); otherwise terminal → surfaced as `{error: "codex error: …"}` (the #204 parking-lot structured signal). Fixed spurious `UNKNOWN_NOTIFICATION` drift on flaky streams (affected `codex_run` too).
...
**Dispatcher**: v2 `main()` and `cc_read_fn` both read CC frames through the module-singleton `_cc_stream` (class `CCStream`: `os.read` + `JsonRpcStream` into a queue) — NEVER `sys.stdin.readline()` (#264). `main()` blocks between turns with `next_frame(None)` (the first/only stdin reader); `cc_read_fn` reads elicitation replies mid-turn with `next_frame(timeout)`; `Reactor.pump(watch_cc=True)` drains the SAME singleton — so `os.read` and `readline()` never mix on the fd, and a burst of frames in one CC write can't strand a 2nd frame in a `TextIOWrapper` buffer. `cc_write_fn` writes CC-facing JSON-RPC 2.0 frames (with `"jsonrpc":"2.0"`) to stdout; the app-server gets jsonrpc_lite frames (no `"jsonrpc"` key) — asymmetric by design.

**Module singleton**: `_v2_manager` (AppServerManager) + `_v2_state_machine` (TurnStateMachine) are module-level singletons. The serial dispatcher loop is the primary concurrency protection (MCP calls arrive one at a time); the busy guard (`TurnStateMachine.is_busy()`) is defense-in-depth and is rarely/never hit in production. **Consequence for callers: two `codex_run` dispatched in ONE assistant turn execute SERIALLY, not concurrently** — the singleton serializes them, so wall-clock = the SUM of both runs, not the max (empirically session `04ad23aa`, 2026-06-21: two `codex_run` issued in one model response ran back-to-back, 294s + 200s, the 2nd codex started <1s after the 1st *finished*; total ≈ 8m15s = sum, not ≈5m = max). Parallel-looking dispatch ≠ parallel execution. For genuine parallelism spawn separate processes — CLI `codex exec` in background, or distinct sessions; this MCP bridge cannot run two codex turns at once. `.mcp.json` = `{"codex": {"command": "python3", "args": ["${CLAUDE_PLUGIN_ROOT}/mcp/codex_server.py"], "env": {}}}`. The server requires **Python 3.11+** (`tomllib`); since that bare `python3` may resolve to an older interpreter, a startup guard (`_python_version_error`) fails loud with a clear stderr message instead of a cryptic `ModuleNotFoundError: tomllib` (#256).
