...
- `additionalContext`: Optional separate text for Claude AI (jhookify enhancement)

**Dual audience (jhookify):** The markdown body is shown to the **user**. Claude AI sees `additionalContext` (if provided) or `[rule-name] markdown-body` as fallback (with `[Hookify Warning]` prefix for warns). Works for PreToolUse, PostToolUse, and UserPromptSubmit events. Stop events do not pass custom context to Claude.
