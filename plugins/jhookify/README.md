# jhookify - Enhanced Hookify Plugin

Enhanced fork of hookify with critical fixes for Claude AI visibility and global rules support.

## Overview

jhookify makes it simple to create hooks without editing complex `hooks.json` files. It creates lightweight markdown configuration files that define patterns to watch for and messages to show when those patterns match.

**Key features (inherited from hookify):**
...
**jhookify enhancements (vs upstream hookify):**
- **Claude sees block reasons** - `permissionDecisionReason` for PreToolUse deny (Fix 1)
- **Claude sees warnings** - `additionalContext` for all warn rules (Fix 2+3)
- **UserPromptSubmit block works** - correct `decision`+`reason` format (Fix 4)
- **UserPromptSubmit rules work** - bug fix: `prompt` field support (Fix 5)
- **PostToolUse block works** - correct `decision`+`reason` format (Fix 6)
- **Custom Claude context** - `additionalContext` YAML field for separate Claude text (Fix 7)
- **Global rules** - `~/.claude/hookify.*.local.md` with local override by name (PR #5)

**Upstream issues resolved:** [#12446](https://github.com/anthropics/claude-code/issues/12446), [#14281](https://github.com/anthropics/claude-code/issues/14281), [#15203](https://github.com/anthropics/claude-code/issues/15203)
...
**Action field:**
- `warn`: Shows warning to user AND Claude (via `additionalContext`). Allows operation.
- `block`: Prevents operation. Claude sees reason via `permissionDecisionReason` (PreToolUse) or `reason` (PostToolUse/UserPromptSubmit).

**additionalContext field (jhookify):**
Optional. Provides separate text for Claude AI (different from user-facing message body):
```yaml
additionalContext: "Use uv add instead of pip install. Never use pip."
```
If omitted, Claude sees the markdown message body.

### Global Rules (jhookify)

Rules in `~/.claude/hookify.*.local.md` apply to ALL projects.
Local rules (`.claude/hookify.*.local.md`) override global by `name:` field.

To disable a global rule locally, create a local rule with same `name:` and `enabled: false`.
