...
class RuleEngine:
    ...
    def evaluate_rules(self, rules: List[Rule], input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Enhanced (jhookify): Claude sees block reasons via permissionDecisionReason
        and warnings via additionalContext. Supports custom additionalContext
        from rule YAML frontmatter.
        ...
        if blocking_rules:
            ...
            # Build Claude-facing context: prefer custom additionalContext, fallback to message
            context_parts = []
            for r in blocking_rules:
                context_parts.append(
                    r.additional_context if r.additional_context else f"[{r.name}] {r.message}"
                )
            claude_context = "\n\n".join(context_parts)
            ...
            if hook_event == 'Stop':
                ...
                # Fix 1: permissionDecisionReason so Claude sees WHY it was blocked
                return {
                    "hookSpecificOutput": {
                        "hookEventName": "PreToolUse",
                        "permissionDecision": "deny",
                        "permissionDecisionReason": claude_context
                    },
                    "systemMessage": combined_message
                }
            elif hook_event == 'PostToolUse':
                # Fix 6: PostToolUse uses decision+reason, NOT permissionDecision
                return {
                    "decision": "block",
                    "reason": claude_context,
                    "hookSpecificOutput": {
                ...
                # Fix 4: UserPromptSubmit block with decision+reason+additionalContext
                return {
                    "decision": "block",
                    "reason": combined_message,
                    "hookSpecificOutput": {
        ...
        if warning_rules:
            ...
            claude_context = "[Hookify Warning]\n" + "\n\n".join(context_parts)

            result = {
                "systemMessage": combined_message
            }

            # Fix 2+3: additionalContext so Claude sees warnings (all event types)
    ...
    def _extract_field(self, field: str, tool_name: str,
        ...
        if input_data:
            ...
            if field == 'reason':
                ...
                # Fix 5: Claude Code sends 'prompt', not 'user_prompt' — support both
